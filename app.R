...
server <- function(input, output, session) {
  ...
  confirmacion_subida <- function(value) {
    if(value){
      ...
      write.csv(df_referencia_L_V, paste(getwd(),"/REFERENCIA_paradas_bus_plasencia_L_V.csv",sep = ""), row.names = FALSE)
      ...
      write.csv(df_csv, paste(getwd(),"/paradas_bus_plasencia_L_V.csv",sep = ""), row.names = FALSE)

      # Referencia de S - D - F
      df_referencia_S_D_F <-  datos$df_referencia_S_D_F
      # Escritura CSV en APP SHINY
      write.csv(df_referencia_S_D_F, paste(getwd(),"/REFERENCIA_paradas_bus_plasencia_S_D_F.csv",sep = ""), row.names = FALSE)
      # Escritura CSV en CARPETA COMPARTIDA para programa tiempos de llegada
      df_csv <- df_referencia_S_D_F[,c("id","Nombre_plataforma","latitud","longitud","linea_1","linea_2","linea_3","sentido")]
      colnames(df_csv)[2] <- "name"
      write.csv(df_csv, paste(getwd(),"/paradas_bus_plasencia_S_D_F.csv",sep = ""), row.names = FALSE)
